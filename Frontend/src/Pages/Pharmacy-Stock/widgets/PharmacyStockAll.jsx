...
function PharmacyStockAll({ pharmacyStock, loading, setPharmacyStockOne, pharmacyLocation, orderID }) {
	const handleEditMedicine = (pharmacy) => {
		setPharmacyStockOne(pharmacy);
	};
	const handleDeleteMedicine = async (id) => {
		try {
			...
			const response = await DELETE(`/pharmacy-stock/${id}`);
	...
	const handleViewDetails = (medicineId) => {
		// Find the selected pharmacy from the pharmacyStock array
		const pharmacy = pharmacyStock.find((pharmacy) => pharmacy.id === medicineId);
	...
	return (
		<div className="flex flex-col items-center justify-center">
			<h1 className="text-2xl font-bold mb-4">All Pharmacy Stocks</h1>
			{loading ? (
				<div>Loading...</div>
			) : (
						...
						<tbody>
							{pharmacyStock.map((pharmacy) => (
								...
								>
									...
									<td className="px-4 py-2 border">{pharmacy?.stock ? pharmacy?.stock : 'N/A'}</td>
									<td className="px-4 py-2 border">
									...
									<div>
										...
										<div className="mt-2">
											<p className="text-sm text-gray-500">
												Here are the details for the selected Pharmacy Stock:
											</p>
											{selectedMedicine && (
												<div className="mt-4 space-y-2">
													...
													<p>
														...
														{selectedMedicine?.stock ? selectedMedicine?.stock : 'N/A'}
													</p>
												</div>
											)}
										</div>
...
export default PharmacyStockAll;
